...
public class ItemHolyCloak extends ItemBauble implements IBaubleRender {
	...
	public ItemHolyCloak(int id, String name) {
		super(id, name);
		LivingHurtEvent.EVENT.register(this::onPlayerDamage);
	}
	...
	public boolean effectOnDamage(LivingHurtEvent event, EntityPlayer player, ItemStack stack) {
		if(!event.source.isMagicDamage()) {
			...
			for(int i = 0; i < 30; i++) {
				...
				Botania.getProxy().sparkleFX(player.worldObj, x, y, z, yellow ? 1F : 0.3F, yellow ? 1F : 0.3F, yellow ? 0.3F : 1F, 0.8F + (float) Math.random() * 0.4F, 3);
			}
			return true;
		}
