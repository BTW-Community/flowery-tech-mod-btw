...
//		FMLCommonHandler.instance().bus().register(this);
		...
		ItemCraftedEvent.EVENT.register(this::onItemCrafted);
	}
	...
	public ItemStack onItemRightClick(ItemStack stack, World world, EntityPlayer player) {
		...
		if(segment == 0)
			player.openGui(Botania.instance.getModID(), LibGuiIDs.CRAFTING_HALO, world, 0, 0, 0);
		else {
			if(itemForPos == null)
		...
		for(int seg = 0; seg < SEGMENTS; seg++) {
			...
			if(slotStack != null) {
				...
				if(slotStack.getItem() instanceof ItemBlock && RenderBlocks.doesRenderIDRenderItemIn3D(Blocks.getBlockFromItem(slotStack.getItem()).getRenderType())) {
					...
					int renderPass = 0;
					do {
						Icon icon = slotStack.getItem().getIcon(slotStack, renderPass);
						if(icon != null) {
