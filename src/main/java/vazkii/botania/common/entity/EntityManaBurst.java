	...
	public void particles() {
		...
		if(fake) {
			...
			if(!noParticles && shouldDoFakeParticles())
				Botania.getProxy().sparkleFX(worldObj, posX, posY, posZ, r, g, b, 0.4F * size, 1, true);
		} else {
			boolean monocle = Botania.getProxy().isClientPlayerWearingMonocle();
			if(monocle)
				Botania.getProxy().setWispFXDepthTest(false);

			if(ConfigHandler.subtlePowerSystem)
				Botania.getProxy().wispFX(worldObj, posX, posY, posZ, r, g, b, 0.1F * size, (float) (Math.random() - 0.5F) * 0.02F, (float) (Math.random() - 0.5F) * 0.02F, (float) (Math.random() - 0.5F) * 0.01F);
			else {
				float or = r;
				float og = g;
				float ob = b;
				...
				do {
					...
					Botania.getProxy().wispFX(worldObj, posX, posY, posZ, r, g, b, 0.2F * size, (float) -motionX * 0.01F, (float) -motionY * 0.01F, (float) -motionZ * 0.01F);
				...
				Botania.getProxy().wispFX(worldObj, posX, posY, posZ, r, g, b, 0.1F * size, (float) (Math.random() - 0.5F) * 0.06F, (float) (Math.random() - 0.5F) * 0.06F, (float) (Math.random() - 0.5F) * 0.06F);
	...
	protected void onImpact(MovingObjectPosition movingobjectposition) {
		...
		if(dead && !isDead) {
			if(!fake) {
				...
				if(!ConfigHandler.subtlePowerSystem)
					for(int i = 0; i < 4; i++)
						Botania.getProxy().wispFX(worldObj, posX, posY, posZ, r, g, b, 0.15F * size, (float) (Math.random() - 0.5F) * 0.04F, (float) (Math.random() - 0.5F) * 0.04F, (float) (Math.random() - 0.5F) * 0.04F);
				Botania.getProxy().sparkleFX(worldObj, (float) posX, (float) posY, (float) posZ, r, g, b, 4, 2);
			}
