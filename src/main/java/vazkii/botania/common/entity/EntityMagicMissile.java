...
//		ReflectionHelper.setPrivateValue(EntityThrowable.class, this, thrower, LibObfuscation.THROWER);
	...
	public void onUpdate() {
		...
		Botania.getProxy().setSparkleFXCorrupt(evil);
		for(int i = 0; i < steps; i++) {
			Botania.getProxy().sparkleFX(worldObj, particlePos.x, particlePos.y, particlePos.z, 1F, evil ? 0F : 0.4F, 1F, 0.8F, 2);
			if(worldObj.rand.nextInt(steps) <= 1)
				Botania.getProxy().sparkleFX(worldObj, particlePos.x + (Math.random() - 0.5) * 0.4, particlePos.y + (Math.random() - 0.5) * 0.4, particlePos.z + (Math.random() - 0.5) * 0.4, 1F, evil ? 0F : 0.4F, 1F, 0.8F, 2);

			particlePos.add(step);
		}
		Botania.getProxy().setSparkleFXCorrupt(false);
