	...
	public void onEntityUpdate() {
		...
		for(int i = 0; i < Math.min(90, ticksExisted); i++) {
			...
			if(worldObj.rand.nextInt(ticksExisted < 90 ? 8 : 20) == 0) {
				...
				Botania.getProxy().wispFX(worldObj, posX + x, posY - 0.2, posZ + z, 1F, (float) Math.random() * 0.25F, (float) Math.random() * 0.25F, 0.65F + (float) Math.random() * 0.45F, (float) (Math.random() - 0.5F) * 0.15F, 0.055F + (float) Math.random() * 0.025F, (float) (Math.random() - 0.5F) * 0.15F);
				...
				Botania.getProxy().wispFX(worldObj, posX + x, posY - 0.2, posZ + z, gs, gs, gs, 0.65F + (float) Math.random() * 0.45F, -0.155F - (float) Math.random() * 0.025F);
			}
		}
